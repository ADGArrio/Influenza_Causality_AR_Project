...
weather_path = "/Users/shreyan/Downloads/VAR/Influenza_Causality_AR_Project/Files/Raw Data/Singapore/singapore_weather_weekly_last25yrs.csv"
...
# Filter FluNet data to Singapore only (handle both uppercase and lowercase column names)
country_col = "COUNTRY_CODE" if "COUNTRY_CODE" in flu.columns else "country_code"
if country_col in flu.columns:
    flu = flu[flu[country_col].astype(str).str.upper() == "SGP"].copy()
...
output_path_a = f"{output_dir}/SG_Training_Data_INF_A.csv"
merged_a.to_csv(output_path_a, index=False)
print(f"\nSaved INF_A dataset to: {output_path_a}")

output_path_b = f"{output_dir}/SG_Training_Data_INF_B.csv"
