...
weather_path = "/Users/shreyan/Downloads/VAR/Influenza_Causality_AR_Project/Files/Raw Data/Qatar/Qatar_weather_weekly_last25yrs.csv"
...
# Filter FluNet data to Qatar only (handle both uppercase and lowercase column names)
country_col = "COUNTRY_CODE" if "COUNTRY_CODE" in flu.columns else "country_code"
if country_col in flu.columns:
    flu = flu[flu[country_col].astype(str).str.upper() == "QAT"].copy()
    print("Flu rows after Qatar filter:", flu.shape)
else:
...
DROP_COLS = [
    ...
    "altimeter",              # Redundant with sea_level_pressure
    "isoyw",
    "iso_weekstartdate",
...
likely_relevant = [
    ...
    "sea_level_pressure",     # Barometric pressure affects respiratory health (5.6% NaN - will interpolate)
    "visibility",             # Air quality/pollution indicator
    "wet_bulb_temperature",   # Heat stress metric (5.6% NaN - will interpolate)
    # Precipitation
...
output_path_a = f"{output_dir}/Qatar_Training_Data_INF_A.csv"
merged_a.to_csv(output_path_a, index=False)
print(f"\nSaved INF_A dataset to: {output_path_a}")

output_path_b = f"{output_dir}/Qatar_Training_Data_INF_B.csv"
